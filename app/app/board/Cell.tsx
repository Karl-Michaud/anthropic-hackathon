...
interface CellProps {
  cell: CellData;
  isDragging: boolean;
  onMouseDown: (e: MouseEvent<HTMLDivElement>, cellId: string, x: number, y: number) => void;
  onTextChange: (cellId: string, newText: string) => void;
}

export default function Cell({ cell, isDragging, onMouseDown, onTextChange }: CellProps) {
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <p className="text-sm text-gray-700 whitespace-pre-wrap break-words">{cell.text}</p>
      )}
    </div>
  );
}
